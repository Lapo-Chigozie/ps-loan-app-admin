...
export class AppMonthlyNetPayDtService 
{
  ...
  private async LoadClientNetPayHeader(ClientId:any , PagenationFilter:any): Promise<void> 
  {
             try
             {
                  // GetLoanAppDetails
     ...
     this.LapoLoanService.GetMonthsNetPayConnector(PagenationFilter).subscribe({
     next:(res)=>{
     ...
     error:(err:any)=>
     {
        ...
        return;
             ...
             catch(exo:any)
             {
              ...
              // console.log("no continue " + err);
              this.loadingService.setLoading(false);
              Swal.fire({ title: 'Error!', text: "An error occurred: " + exo.message, icon: 'error', confirmButtonText: 'Ok' })
              return;
             }
  }
