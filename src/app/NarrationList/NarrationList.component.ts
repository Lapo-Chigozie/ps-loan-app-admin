...
@Component({
  selector: 'app-NarrationList',
  templateUrl: './NarrationList.component.html',
  styleUrls: ['./NarrationList.component.css']
})
export class NarrationListComponent extends AppBasedComponent implements OnInit 
{
  ...
  public onNewNarrationsApp() : void
  {
      this.onNaviagateBack("/narrations");
      return;
  }
  ...
  private async GetAllLoanApplys(): Promise<void> 
  {
        ...
        await this.LapoLoanService.GetListOfNarrationListList(parseInt(this.AcctId)).subscribe({
        next:(res)=>{
