...
@Component({
  selector: 'app-repaymentloans',
  templateUrl: './repaymentloans.component.html',
  styleUrls: ['./repaymentloans.component.css']
})

export class RepaymentloansComponent extends AppBasedComponent implements OnInit
{
  private AppId:any;
  ...
  public async UploadFile(files:any): Promise<void>
  {
        ...
        try
        {
                  this.fileUploadService.UploadRepaymentLoans(files).subscribe({
                  next: (event) => 
                  {
                    this.loadingService.setLoading(false);
